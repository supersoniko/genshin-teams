...
export default {
	...
	prompts: [
		...
		},
		{
			type: 'confirm',
			name: 'hasProps',
			message: 'Would you like to add props to your template?',
			default: true
		},
		{
			type: 'checkbox',
		...
		},
		{
			type: 'confirm',
			name: 'pure',
			message: 'Do you prefer a pure template?',
			default: false
		},
		{
			type: 'confirm',
	...
	actions: ({name, options, addToIndex}: PromptProps): any[] => {
		...
		actions.push({
			...
			base: 'conf/plop/templates/template',
			templateFiles: [
				'conf/plop/templates/template/index.tsx.hbs',
				options.includes('style') && 'conf/plop/templates/template/style.hook.ts.hbs',
				options.includes('test') && 'conf/plop/templates/template/test.tsx.hbs',
				options.includes('story') && 'conf/plop/templates/template/index.stories.tsx.hbs'
			].filter(Boolean),
			data: {
		...
		if (addToIndex) {
			...
			actions.push({
				...
				template: `export { default as {{ pascalCase name }}Template } from './{{ pascalCase name }}'`
			})
